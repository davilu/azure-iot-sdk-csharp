    ...
    internal sealed partial class IotHubConnectionString : IAuthorizationProvider
    {
        ...
        public IotHubConnectionString(IotHubConnectionStringBuilder builder)
        {
            ...
            if (builder.AuthenticationMethod is AuthenticationWithTokenRefresh)
            {
                ...
                if (Logger.IsEnabled) Logger.Info(this, $"{nameof(IAuthenticationMethod)} is {nameof(AuthenticationWithTokenRefresh)}: {Logging.IdOf(TokenRefresher)}");
            ...
            else if (!string.IsNullOrEmpty(SharedAccessKey))
            {
                if (ModuleId.IsNullOrWhiteSpace())
                {
                    ...
                    if (Logger.IsEnabled) Logger.Info(this, $"{nameof(IAuthenticationMethod)} is {nameof(DeviceAuthenticationWithSakRefresh)}: {Logging.IdOf(TokenRefresher)}");
                }
                else
                {
                    TokenRefresher = new ModuleAuthenticationWithSakRefresh(DeviceId, ModuleId, this) as AuthenticationWithTokenRefresh;
                    if (Logger.IsEnabled) Logger.Info(this, $"{nameof(IAuthenticationMethod)} is {nameof(ModuleAuthenticationWithSakRefresh)}: {Logging.IdOf(TokenRefresher)}");
                }
