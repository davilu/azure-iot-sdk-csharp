...
namespace Microsoft.Azure.Devices.Client.Transport.Amqp
{
    internal class AmqpAuthenticationRefresher : IAmqpAuthenticationRefresher, IDisposable
    {
        ...
        internal AmqpAuthenticationRefresher(DeviceIdentity deviceIdentity, AmqpIoTCbsLink amqpCbsLink)
        {
            ...
            if (Logger.IsEnabled) Logger.Associate(this, deviceIdentity, $"{nameof(DeviceIdentity)}");
            if (Logger.IsEnabled) Logger.Associate(this, amqpCbsLink, $"{nameof(_amqpIoTCbsLink)}");
        }

        public async Task InitLoopAsync(TimeSpan timeout)
        {
            if (Logger.IsEnabled) Logger.Enter(this, timeout, $"{nameof(InitLoopAsync)}");
            ...
            if (Logger.IsEnabled) Logger.Exit(this, timeout, $"{nameof(InitLoopAsync)}");
        }

        public void StartLoop(DateTime refreshOn, CancellationToken cancellationToken)
        {
            if (Logger.IsEnabled) Logger.Enter(this, refreshOn, $"{nameof(StartLoop)}");
            _refreshLoop = RefreshLoopAsync(refreshOn, cancellationToken);
            if (Logger.IsEnabled) Logger.Exit(this, refreshOn, $"{nameof(StartLoop)}");
        }
        ...
        private async Task RefreshLoopAsync(DateTime refreshesOn, CancellationToken cancellationToken)
        {
            ...
            {
                if (Logger.IsEnabled) Logger.Info(this, refreshesOn, $"Before {nameof(RefreshLoopAsync)}");
                ...
                if (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    {
                        if (Logger.IsEnabled) Logger.Info(this, refreshesOn, $"Refresh token failed {ex}");
                    }
                    finally
                    {
                        if (Logger.IsEnabled) Logger.Info(this, refreshesOn, $"After {nameof(RefreshLoopAsync)}");
                    }
        ...
        {
            if (Logger.IsEnabled) Logger.Info(this, $"{nameof(StopLoop)}");
