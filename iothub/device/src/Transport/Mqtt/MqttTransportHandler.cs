...
namespace Microsoft.Azure.Devices.Client.Transport.Mqtt
{
    ...
    sealed class MqttTransportHandler : TransportHandler, IMqttIotHubEventHandler
    {
        ...
        public override Task SendEventAsync(Message message, CancellationToken cancellationToken)
        {
            try
            {
                if (Logger.IsEnabled) Logger.Enter(this, cancellationToken, nameof(SendEventAsync));
                cancellationToken.ThrowIfCancellationRequested();
            ...
            {
                if (Logger.IsEnabled) Logger.Exit(this, cancellationToken, nameof(SendEventAsync));
            }
        }
        ...
        {
            if (Logger.IsEnabled) Logger.Enter(this, timeoutHelper, timeoutHelper.RemainingTime(), $"{nameof(ReceiveAsync)}");
            ...
            if (Logger.IsEnabled) Logger.Exit(this, timeoutHelper, timeoutHelper.RemainingTime(), $"{nameof(ReceiveAsync)}");
        ...
        public override async Task CloseAsync(CancellationToken cancellationToken)
        {
            try
            {
                if (Logger.IsEnabled) Logger.Enter(this, "", $"{nameof(MqttTransportHandler)}.{nameof(CloseAsync)}");
            ...
            {
                if (Logger.IsEnabled) Logger.Exit(this, "", $"{nameof(MqttTransportHandler)}.{nameof(CloseAsync)}");
            }
        }
        ...
        public async void OnError(Exception exception)
        {
            ...
            {
                if (Logger.IsEnabled) Logger.Error(this, ex.ToString(), nameof(OnError));
            }
        }
        ...
        public override async Task<Twin> SendTwinGetAsync(CancellationToken cancellationToken)
        {
            ...
            using (var response = await SendTwinRequestAsync(request, rid, cancellationToken).ConfigureAwait(true))
            {
                ...
                using (StreamReader reader = new StreamReader(response.GetBodyStream(), System.Text.Encoding.UTF8))
                {
                    ...
                    {
                        if (Logger.IsEnabled) Logger.Error(this, $"Failed to parse Twin JSON: {ex}. Message body: '{body}'");
                        throw;
                    }
                }
            }
        ...
        Func<IPAddress[], int, Task<IChannel>> CreateChannelFactory(IotHubConnectionString iotHubConnectionString, MqttTransportSettings settings, ProductInfo productInfo)
        {
            return async (addresses, port) =>
            {
                ...
                foreach (IPAddress address in addresses)
                {
                    try
                    {
                        if (Logger.IsEnabled) Logger.Info(this, $"Connecting to {address.ToString()}", nameof(CreateChannelFactory));
                    ...
                    catch (AggregateException ae)
                    {
                        ae.Handle((ex) =>
                        {
                            ...
                            {
                                if (Logger.IsEnabled) Logger.Error(this, $"ConnectException trying to connect to {address.ToString()}: {ex.ToString()}", nameof(CreateChannelFactory));
                                return true;
                            }
            ...
            return async (address, port) =>
            {
                ...
                try
                {
                    if (webProxy != DefaultWebProxySettings.Instance)
                    {
                        ...
                        if (Logger.IsEnabled)
                        {
                            Logger.Info(this, $"{nameof(CreateWebSocketChannelFactory)} Setting ClientWebSocket.Options.Proxy");
                        }
                    }
                }
                catch (PlatformNotSupportedException)
                {
                    ...
                    if (Logger.IsEnabled)
                    {
                        Logger.Error(this, $"{nameof(CreateWebSocketChannelFactory)} PlatformNotSupportedException thrown as .NET Core 2.0 doesn't support proxy");
                    }
                }
        ...
        static IEventLoopGroup GetEventLoopGroup()
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(envValue))
                {
                    ...
                    {
                        if (Logger.IsEnabled) Logger.Info(null, $"EventLoopGroup threads count {processorThreadCount}.");
            ...
            {
                if (Logger.IsEnabled) Logger.Info(null, $"Could not read EventLoopGroup threads count {ex}");
                return new MultithreadEventLoopGroup();
            }

            if (Logger.IsEnabled) Logger.Info(null, "EventLoopGroup threads count was not set.");
            return new MultithreadEventLoopGroup();
        }
    }
}
