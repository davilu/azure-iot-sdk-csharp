...
namespace Microsoft.Azure.Devices.Provisioning.Client.Transport
{
    internal class AmqpClientConnection
    {
        ...
        {
            if (Logger.IsEnabled) Logger.Enter(this, $"{nameof(AmqpClientConnection)}.{nameof(OpenAsync)}");
            ...
            if (useWebSocket)
            {
                ...
                if (provider != null)
                {
                    if (Logger.IsEnabled) Logger.Info(this, $"{nameof(AmqpClientConnection)}.{nameof(OpenAsync)}: Using SaslTransport");
                    ...
                    if (Logger.IsEnabled) Logger.Info(this, $"{nameof(AmqpClientConnection)}.{nameof(OpenAsync)}: Sent Protocol Header: {_sentHeader.ToString()} operationPending: {operationPending} completedSynchronously: {args.CompletedSynchronously}");
        ...
        {
            if (Logger.IsEnabled) Logger.Enter(this, $"{nameof(AmqpClientConnection)}.{nameof(OnWriteHeaderComplete)}");
        ...
        {
            if (Logger.IsEnabled) Logger.Enter(this, $"{nameof(AmqpClientConnection)}.{nameof(OnReadHeaderComplete)}");
            ...
            try
            {
                ...
                if (Logger.IsEnabled) Logger.Info(this, $"{nameof(AmqpClientConnection)}.{nameof(OnReadHeaderComplete)}: Received Protocol Header: {receivedHeader.ToString()}");
                ...
                if (Logger.IsEnabled) Logger.Info(this, $"{nameof(AmqpClientConnection)}.{nameof(OnReadHeaderComplete)}: Created SaslTransportHandler ");
        ...
        {
            if (Logger.IsEnabled) Logger.Enter(this, $"{nameof(AmqpClientConnection)}.{nameof(CompleteOnException)}");

            if (args.Exception != null && args.Transport != null)
            {
                if (Logger.IsEnabled) Logger.Error(this, $"{nameof(AmqpClientConnection)}.{nameof(CompleteOnException)}: Exception thrown {args.Exception.Message}");
